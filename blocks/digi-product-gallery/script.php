<?php
/**
 * DigiCommerce Product Gallery Block JavaScript
 *
 * @package DigiBlocks
 */
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function createLightbox() {
        ...
        lightboxEl.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.85); display: flex; align-items: center; justify-content: center; z-index: 999999; opacity: 0; visibility: hidden; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); backdrop-filter: blur(4px);';
        ...
        lightboxEl.innerHTML = '<div style="position: relative; max-width: 90%; max-height: 90%; display: flex; align-items: center; justify-content: center;">' +
            '<img style="max-width: 100%; max-height: 85vh; object-fit: contain; border-radius: 12px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25);" src="" alt="" />' +
            '<button type="button" style="position: absolute; top: -60px; right: 0; background: transparent; border-radius: 50%; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); outline: none;" aria-label="Close">' +
                getCloseIcon() +
            '</button>' +
            (allImages.length > 1 ? 
                '<button type="button" style="position: absolute; left: -70px; top: 50%; transform: translateY(-50%); background: transparent; border-radius: 50%; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); outline: none;" aria-label="Previous">' +
                    getLeftIcon() +
                '</button>' +
                '<button type="button" style="position: absolute; right: -70px; top: 50%; transform: translateY(-50%); background: transparent; border-radius: 50%; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); outline: none;" aria-label="Next">' +
                    getRightIcon() +
                '</button>' +
                '<div style="position: absolute; bottom: -60px; left: 50%; transform: translateX(-50%); background: rgba(255, 255, 255, 0.95); padding: 10px 20px; border-radius: 24px; font-size: 14px; font-weight: 500; color: #333; backdrop-filter: blur(10px); border: 2px solid rgba(255, 255, 255, 0.8);">' +
