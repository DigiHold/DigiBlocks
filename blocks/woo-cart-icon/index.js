...
import WooCartIconEdit from './edit';
import WooCartIconSave from './save';
import { getBlockActiveStatus, isWooActive } from '../../resources/js/blocks/utils.js';

if ( isWooActive() ) {
	/**
	 * Register Menu Cart block
	 */
	registerBlockType('digiblocks/woo-cart-icon', {
		apiVersion: 2,
		title: digiBlocksData.blocks['woo-cart-icon'].title,
		category: 'digiblocks-woocommerce',
		icon: {
			src: () => {
				const { viewbox, path } = digiBlocksData.blocks['woo-cart-icon'].icon;
				return (
		...
		description: digiBlocksData.blocks['woo-cart-icon'].description,
		keywords: [__('cart', 'digiblocks'), __('menu', 'digiblocks'), __('woocommerce', 'digiblocks'), __('shop', 'digiblocks')],
		supports: {
			inserter: getBlockActiveStatus('woo-cart-icon') ? true : false,
			html: false,
			className: false,
		...
		edit: WooCartIconEdit,
		save: WooCartIconSave,
	});
}