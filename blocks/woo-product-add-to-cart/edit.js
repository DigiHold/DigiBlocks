...
/**
 * Edit function for the Product Add To Cart block
 */
const WooProductAddToCartEdit = ({ attributes, setAttributes, clientId }) => {
    const {
        id,
        anchor,
        visibility,
    ...
    const generateCSS = () => {
        ...
        return `
            ...
            .${id} .digiblocks-add-to-cart-container {
                display: flex;
                align-items: ${showQuantityInput ? 'center' : 'flex-start'};
                gap: 10px;
                justify-content: ${buttonAlignment};
            }
            
            ${showQuantityInput ? `
            .${id} .digiblocks-quantity-input {
                width: ${quantityInputWidth[activeDevice]}px;
                padding: 8px 12px;
                background-color: ${quantityBackgroundColor};
                color: ${quantityTextColor};
                border: 1px solid ${quantityBorderColor};
                ${getDimensionCSS(quantityBorderRadius, 'border-radius', activeDevice)}
                font-size: 14px;
                text-align: center;
                transition: all 0.3s ease;
            }
            
            .${id} .digiblocks-quantity-input:focus {
                outline: none;
                border-color: ${backgroundColor};
                box-shadow: 0 0 0 1px ${backgroundColor}40;
            }
            ` : ''}
            
            .${id} .digiblocks-add-to-cart-button {
                display: inline-flex;
                align-items: center;
            ...
            .${id} .digiblocks-add-to-cart-button:hover {
                background-color: ${backgroundHoverColor || backgroundColor};
                color: ${textHoverColor || textColor};
                transform: translateY(-1px);
            }
            
            .${id} .digiblocks-add-to-cart-button:active {
                transform: translateY(0);
            }
            
            .${id} .digiblocks-add-to-cart-button svg {
                width: 1em;
                height: 1em;
                margin-right: 8px;
                fill: currentColor;
                ...
                return (
                    <>
                        ...
                        >
                            ...
                            >
                                {__('This is a placeholder. The actual add to cart button will be displayed on single product pages.', 'digiblocks')}
                            </Notice>
                        </TabPanelBody>
                        ...
                        >
                            ...
                            <ToggleControl
                                label={__('Show Quantity Input', 'digiblocks')}
                                checked={showQuantityInput}
                                onChange={(value) => setAttributes({ showQuantityInput: value })}
                                help={__('Display quantity input field next to the add to cart button', 'digiblocks')}
                                __nextHasNoMarginBottom={true}
                            />
                            
                            {showQuantityInput && (
                                <ResponsiveControl
                                    label={__('Quantity Input Width', 'digiblocks')}
                                >
                                    <RangeControl
                                        value={quantityInputWidth[localActiveDevice]}
                                        onChange={(value) =>
                                            setAttributes({
                                                quantityInputWidth: {
                                                    ...quantityInputWidth,
                ...
                return (
                    <>
                                        ...
                                        return (
                                            <PanelColorSettings
                                                ...
                                                colorSettings={[
                                                    ...
                                                    ...(showQuantityInput ? [
                                                        {
                                                            value: quantityBackgroundColor,
                                                            onChange: (value) => setAttributes({ quantityBackgroundColor: value }),
                                                            label: __("Quantity Background", "digiblocks"),
                                                        },
                                                        {
                                                            value: quantityTextColor,
                                                            onChange: (value) => setAttributes({ quantityTextColor: value }),
                                                            label: __("Quantity Text", "digiblocks"),
                                                        },
                                                        {
                                                            value: quantityBorderColor,
                                                            onChange: (value) => setAttributes({ quantityBorderColor: value }),
                                                            label: __("Quantity Border", "digiblocks"),
                                                        }
                                                    ] : [])
                                                ]}
                                            />
                        ...
                        >
                            ...
                            {showQuantityInput && (
                                <ResponsiveControl
                                    label={__('Quantity Border Radius', 'digiblocks')}
                                >
                                    <DimensionControl
                                        values={quantityBorderRadius[localActiveDevice]}
                                        onChange={(value) =>
                                            setAttributes({
                                                quantityBorderRadius: {
                                                    ...quantityBorderRadius,
    ...
    // Build class names
    const blockClasses = `digiblocks-product-add-to-cart ${id} ${customClasses || ""}`;
    ...
    return (
        <>
            ...
            <div {...blockProps}>
                <div className="digiblocks-add-to-cart-container">
                    {showQuantityInput && (
                        <input
                            type="number"
                            className="digiblocks-quantity-input"
                            value="1"
                            min="1"
                            readOnly
                        />
                    )}
                    <button className="digiblocks-add-to-cart-button">
...
export default WooProductAddToCartEdit;