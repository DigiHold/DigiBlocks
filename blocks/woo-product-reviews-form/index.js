...
import WooProductReviewsFormEdit from './edit';
import WooProductReviewsFormSave from './save';
import { getBlockActiveStatus, isWooActive } from '../../resources/js/blocks/utils.js';

if ( isWooActive() ) {
	/**
	 * Register Product Reviews Form block
	 */
	registerBlockType('digiblocks/woo-product-reviews-form', {
		apiVersion: 2,
		title: digiBlocksData.blocks['woo-product-reviews-form'].title,
		category: 'digiblocks-woocommerce',
		icon: {
			src: () => {
				const { viewbox, path } = digiBlocksData.blocks['woo-product-reviews-form'].icon;
				return (
		...
		description: digiBlocksData.blocks['woo-product-reviews-form'].description,
		keywords: [__('product', 'digiblocks'), __('reviews', 'digiblocks'), __('woocommerce', 'digiblocks'), __('rating', 'digiblocks'), __('comments', 'digiblocks')],
		supports: {
			inserter: getBlockActiveStatus('woo-product-reviews-form') ? true : false, // Remove the block if disabled
			html: false,
			className: false,
		...
		edit: WooProductReviewsFormEdit,
		save: WooProductReviewsFormSave,
	});
}