/**
 * DigiBlocks Animations Handler
 */
document.addEventListener('DOMContentLoaded', function() {
    // Find all blocks with animations
    const animatedElements = document.querySelectorAll('[class*="animate-"]');
    
    if (animatedElements.length === 0) return;
    
    // Define the animations keyframes only once
    ...
    // Animation configuration mapping
    const animations = {
        'fade-in': {
            ...
            (entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        // Stop observing after animation is triggered (unless it's an infinite animation)
